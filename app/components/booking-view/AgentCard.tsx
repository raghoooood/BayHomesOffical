...
const AgentCard: React.FC<AgentCardProps> = ({ agentImage, agentName, agentPosition, _id }) => {
    ...
    return (
      <div className="bg-white shadow-md rounded-lg overflow-hidden sticky top-5">
        ...
        <div className="flex justify-around items-center p-4 bg-white border-b border-gray-300">
          ...
          <button onClick={() => handleIconClick('/call')} aria-label="Call" className="text-sky-900 bg-gray-200 p-3 rounded-lg duration-300 hover:scale-105">
            <FaPhoneAlt size={22} />
          </button>
          <button onClick={() => handleIconClick('/whatsapp')} aria-label="WhatsApp" className="text-green-600 bg-gray-200 p-3 rounded-lg duration-300 hover:scale-105">
            <FaWhatsapp size={22} />
          </button>
        </div>
